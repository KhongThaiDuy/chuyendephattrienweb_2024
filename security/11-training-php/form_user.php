...
function hashId($id)
{
    return md5($id); // You can also use a different secure hashing function
}
...
if (!empty($_GET['id'])) {
    ...
    foreach ($users as $usr) {
        if (hashId($usr['id']) === $hashId) {
            $_id = $usr['id']; // Retrieve the actual ID if it matches
            $user = $userModel->findUserById($_id); // Get user by actual ID
            break;
        }
    }
}
